...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    df_pan = df[df['Tarifa'].isin(tarifas_analisis.keys())].copy()
    ...
    resid_pan  = df[df['Tarifa']=='RESIDENCIAL'][df['Año'].isin([2020,2021])]['Ventas'].mean()
    resid_rec  = df[df['Tarifa']=='RESIDENCIAL'][df['Año'].isin([2022,2023])]['Ventas'].mean()
    comerc_pan = df[df['Tarifa']=='COMERCIOS Y SERVICIOS'][df['Año'].isin([2020,2021])]['Ventas'].mean()
    comerc_rec = df[df['Tarifa']=='COMERCIOS Y SERVICIOS'][df['Año'].isin([2022,2023])]['Ventas'].mean()
    ...
    with col2:
        ...
        anual_comp = df[df['Tarifa'].isin(['RESIDENCIAL','COMERCIOS Y SERVICIOS'])]\
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    # Datos
    estac_resid = df[df['Tarifa']=='RESIDENCIAL'].groupby(['Estacion','Mes'])['Ventas'].mean().reset_index()
    temp_estac  = df.groupby(['Estacion','Mes'])[['T2M','Ventas']].mean().reset_index()
    temp_emp    = df.groupby(['Empresa','Estacion'])['T2M'].mean().reset_index()
    ventas_mes  = df.groupby('Mes')[['Ventas','T2M']].mean().reset_index()

    # KPIs
    verano_v = df[df['Estacion']=='Verano (Dic-Abr)']['Ventas'].mean()
    lluvia_v = df[df['Estacion']=='Lluvioso (May-Nov)']['Ventas'].mean()
    verano_t = df[df['Estacion']=='Verano (Dic-Abr)']['T2M'].mean()
    lluvia_t = df[df['Estacion']=='Lluvioso (May-Nov)']['T2M'].mean()
    ...
    df_sc = df[df['Tarifa']=='RESIDENCIAL'][['T2M','Ventas','Empresa','Estacion']].dropna()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    guana  = df[df['Empresa']=='COOPEGUANACASTE'].copy()
    cnfl   = df[df['Empresa']=='CNFL'].copy()
    ice    = df[df['Empresa']=='ICE'].copy()
    ...
    with col2:
        ...
        df_temp_comp  = df[df['Empresa'].isin(empresas_comp)]\
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    empresa_sel = st.multiselect(
        ...
        options=sorted(df_f['Empresa'].unique()),
        default=['ICE','CNFL','ESPH','COOPELESCA']
    )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    df_geo, ok = cargar_db("""
        ...
        FROM "Fact_Dim".vw_modelo_predictivo_central_tarifa_mensual
